import{g as e,O as c,as as s}from"./index-DZ1tk7sx.js";const r="fibonacci",a=`#define rcp(x) 1./x
#define GOLDEN_RATIO 1.618033988749895
vec2 Golden2dSeq(int i,float n)
{return vec2(float(i)/n+(0.5/n),fract(float(i)*rcp(GOLDEN_RATIO)));}
vec2 SampleDiskGolden(int i,int sampleCount)
{vec2 f=Golden2dSeq(i,float(sampleCount));return vec2(sqrt(f.x),TWO_PI*f.y);}`;e.IncludesShadersStore[r]||(e.IncludesShadersStore[r]=a);const f={name:r,shader:a},t="diffusionProfile",n="uniform vec3 diffusionS[5];uniform float diffusionD[5];uniform float filterRadii[5];";e.IncludesShadersStore[t]||(e.IncludesShadersStore[t]=n);const d={name:t,shader:n},o="subSurfaceScatteringPixelShader",l=`#include<helperFunctions>
#include<fibonacci>
...
totalWeight=max(totalWeight,HALF_MIN);gl_FragColor=vec4(inputColor.rgb+albedo*max(totalIrradiance/totalWeight,vec3(0.0)),1.);}`;e.ShadersStore[o]||(e.ShadersStore[o]=l);const S=[c,f,s,d];for(const i of S)e.IncludesShadersStore[i.name]||(e.IncludesShadersStore[i.name]=i.shader);const p={name:o,shader:l};export{p as subSurfaceScatteringPixelShader};

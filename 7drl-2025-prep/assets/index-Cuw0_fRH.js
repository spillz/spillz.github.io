...
class WebGLTileWidget extends Widget {
   ...
   */
  addTile(sx, sy, sw, sh, dx, dy, dw, dh, tint) {
  ...
  draw(app, ctx) {
    ...
    for (let { sx, sy, sw, sh, dx, dy, dw, dh, tint } of this.drawBatch) {
...
function positionOrbs(size, numOrbs = 10) {
  const [w, h] = size;
  const orbs = /* @__PURE__ */ new Map();
  let orbNum = 1;
  while (orbs.size < numOrbs) {
    const newPos = getRandomInt(w * h);
    if (!orbs.has(newPos)) {
      orbs.set(newPos, Math.ceil(orbNum / 2));
      orbNum++;
    }
  }
  return orbs;
...
class GLDemo extends App {
  constructor() {
    ...
    /**@type {[number, number]} */
    __publicField(this, "terrainSize", [16, 10]);
    /**@type {Set<number>} */
    __publicField(this, "badOrbs", /* @__PURE__ */ new Set());
    __publicField(this, "lastOrb", -1);
    __publicField(this, "score", 0);
    __publicField(this, "gameOver", false);
    this.orbs = positionOrbs(this.terrainSize);
    this.terrain = new Grid2D(this.terrainSize).fill(0);
    this.visible = new Grid2D(this.terrainSize).fill(0);
    this.webgl = new WebGLTileWidget({
      id: "webgl",
      src: urlTileset,
      txTileDim: 8,
    ...
    this.header = new Label({
      text: `Orbs: ${this.orbs.size}, Score: ${this.score}/30`,
      fontName: "serif",
      align: "center",
  ...
  restart() {
    ...
    let i = 0;
    while (i < 100) {
      const uPos = getRandomInt(this.terrainSize[0] * this.terrainSize[1]);
      const pos = this.intToPos(uPos);
      if (!this.orbs.has(uPos) && this.terrain.get(pos) === mapTiles.GROUND) {
        this.playerPos = pos;
        break;
      }
      i++;
  ...
  updateHeader() {
    if (this.gameOver) {
      this.header.text = `Game over, Score: ${this.score}/30, Restart (R)`;
    } else {
      this.header.text = `Orbs: ${this.orbs.size}, Score: ${this.score}/30`;
    }
  }
  /**
   * 
   ...
   */
  collectOrb(pos) {
    const uPos = this.posToInt(pos);
    const orbVal = this.orbs.get(uPos);
    if (orbVal !== void 0) {
      for (let xyNear of this.terrain.iterInRange(pos, orbVal)) {
        const uPosNear = this.posToInt(xyNear);
        const nearVal = this.orbs.get(uPosNear);
        if (uPosNear !== uPos && nearVal !== void 0 && nearVal < orbVal) {
          this.gameOver = true;
          this.badOrbs.add(uPosNear);
          this.lastOrb = uPos;
        }
      }
      if (!this.gameOver) {
        this.score += orbVal;
        this.orbs.delete(uPos);
      }
      if (this.orbs.size === 0) {
        this.gameOver = true;
      }
  ...
  updateTerrain() {
    ...
    for (let xy of map.iterAll()) {
      map.set(xy, 0);
    }
    for (let [pos, orbSize] of this.orbs.entries()) {
      let xy = (
        /**@type {[number, number]}*/
        this.intToPos(pos)
      );
      for (let gpos of map.iterInRange(xy, orbSize)) {
        const oldValue = map.get(gpos);
        const newValue = Math.min(oldValue + 1, 3);
        map.set(gpos, newValue);
      }
    }
    this.updateVisible();
  }
  updateVisible() {
    for (let xy of this.terrain.iterAll()) {
      if (this.terrain.get(xy) <= 1) {
        this.visible.set(xy, 1);
        for (let xyAdj of this.terrain.iterAdjacent(xy)) {
          this.visible.set(xyAdj, 1);
        }
      }
    }
    for (let xy of this.terrain.iterInRange(this.playerPos, 2)) {
      this.visible.set(xy, 1);
    }
    this.updateTiles();
  }
  updateTiles() {
    ...
    let tint = [1, 1, 1, 1];
    for (let xy of this.terrain.iterAll()) {
      if (this.visible.get(xy) === 1) {
        const ti = tileIcons[this.terrain.get(xy)];
        this.webgl.addTile(ti[0], ti[1], 1, 1, xy[0], xy[1], 1, 1, tint);
      }
    }
    if (this.gameOver) {
      if (this.lastOrb >= 0) {
        const range = this.orbs.get(this.lastOrb);
        const xy = this.intToPos(this.lastOrb);
        if (range !== void 0) {
          for (let xyNear of this.terrain.iterInRange(xy, range)) {
            const ti = tileIcons[mapTiles.FIRE];
            this.webgl.addTile(ti[0], ti[1], 1, 1, xyNear[0], xyNear[1], 1, 1, tint);
          }
        }
      }
    }
    for (let [uPos, orbVal] of this.orbs.entries()) {
      const xy = this.intToPos(uPos);
      if (this.badOrbs.has(uPos)) {
        const ti = tileIcons[mapTiles.BADORB];
        this.webgl.addTile(ti[0], ti[1], 1, 1, xy[0], xy[1], 1, 1, tint);
      } else {
        if (this.visible.get(xy) || new Vec2(xy).dist(this.playerPos) <= 2) {
          const ti = tileIcons[mapTiles.ORB];
          this.webgl.addTile(ti[0], ti[1], 1, 1, xy[0], xy[1], 1, 1, tint);
        }
      }
    }
    this.webgl.addTile(9, 0, 1, 1, this.playerPos[0], this.playerPos[1], 1, 1, tint);
    this.updateHeader();
  }
  on_key_down(e, o, v) {
    ...
    if (this.gameOver) {
      ...
      if (ip.isKeyDown("w") && this.playerPos[1] > 0) {
        this.playerPos[1] -= 1;
      } else if (ip.isKeyDown("s") && this.playerPos[1] < this.terrainSize[1] - 1) {
        this.playerPos[1] += 1;
      } else if (ip.isKeyDown("a") && this.playerPos[0] > 0) {
        this.playerPos[0] -= 1;
      } else if (ip.isKeyDown("d") && this.playerPos[0] < this.terrainSize[0] - 1) {
        this.playerPos[0] += 1;
      } else if (ip.isKeyDown(" ")) ;
    }
    if (this.orbs.has(this.posToInt(this.playerPos))) {
      this.collectOrb(this.playerPos);
    } else {
      this.updateVisible();
    }
    this.webgl.drawBatch[this.webgl.drawBatch.length - 1].dx = this.playerPos[0];
    this.webgl.drawBatch[this.webgl.drawBatch.length - 1].dy = this.playerPos[1];

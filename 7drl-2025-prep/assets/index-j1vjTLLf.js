...
class GLDemo extends App {
   ...
   */
  intToPos(int) {
    return [int % this.terrainSize[0], Math.floor(int / this.terrainSize[0])];
  }
  /**
   * 
   * @param {[number, number]} pos 
  ...
  updateTiles() {
    ...
    for (let [uPos, orbVal] of this.orbs.entries()) {
      ...
      if (new Vec2(xy).dist(this.playerPos) <= 2) {
        const ti = tileIcons[mapTiles.ORB];
        this.webgl.addTile(ti[0], ti[1], 1, 1, xy[0], xy[1], 1, 1, tint);
      }
    }
    this.webgl.addTile(5, 0, 1, 1, this.playerPos[0], this.playerPos[1], 1, 1, tint);
  }
  on_key_down(e, o, v) {

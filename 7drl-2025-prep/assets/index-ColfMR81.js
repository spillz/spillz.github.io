...
class Vec2 extends Array {
   ...
   * @returns {Vec2}
   */
  relativeToPS(pos2, sz) {
    return this.sub(pos2).scale(1 / sz);
  }
  /**
   ...
   * @returns {Vec2}
   */
  absoluteFromPS(pos2, sz) {
    return this.scale(sz).add(pos2);
  }
  /**
...
class Rect extends Array {
   ...
   */
  translate(pos2) {
    return new Rect([this.x + pos2[0], this.y + pos2[1], this.w, this.h]);
  }
  /**
...
class Grid2D extends Array {
   ...
   */
  get(pos2) {
    return this[pos2[0] + pos2[1] * this.tileDim[0]];
  }
  /**
   ...
   */
  set(pos2, val) {
    this[pos2[0] + pos2[1] * this.tileDim[0]] = val;
  }
  /**
  ...
  *iterTypesBetween(pos1, pos2, types) {
    for (var pos3 of this.iterBetween(pos1, pos2)) {
      if (types.includes(this.get(pos3))) {
        yield pos3;
      }
    }
  }
  ...
  *iterTypesInBetween(pos1, pos2, types) {
    for (var pos3 of this.iterInBetween(pos1, pos2)) {
      if (types.includes(this.get(pos3))) {
        yield pos3;
      }
    }
  ...
  hasTypesBetween(pos1, pos2, types) {
    for (var pos3 of this.iterTypesBetween(pos1, pos2, types)) {
      return true;
    }
    return false;
  }
  ...
  hasTypesInBetween(pos1, pos2, types) {
    for (var pos3 of this.iterTypesInBetween(pos1, pos2, types)) {
      return true;
    }
    return false;
  }
  ...
  *iterTypes(types, sub_rect) {
    for (var pos2 of this.iterAll(sub_rect)) {
      if (types.includes(this.get(pos2))) {
        yield pos2;
      }
    }
  }
  ...
  *iterAdjacent(pos2) {
   ...
   * @param {number[]} types 
   */
  hasAdjacent(pos2, types) {
    for (let npos of this.iterAdjacent(pos2)) {
      if (types.includes(this.get(pos2))) return true;
    }
    return false;
  }
  /**
  ...
  *iterInRange(pos2, radius) {
   ...
   */
  *iterTypesInRange(pos2, types, radius, blocker_types = null) {
    for (var pos0 of this.iterInRange(pos2, radius)) {
      if (blocker_types !== null && this.hasTypesBetween(pos2, pos0, blocker_types)) continue;
   ...
   * @param {number[]|null} blocker_types 
   */
  numInRange(pos2, types, radius, blocker_types = null) {
    var num = 0;
    for (var pos0 of this.iterTypesInRange(pos2, types, radius, blocker_types)) {
      num++;
    }
    return num;
  }
  ...
  *iterRect(rect, mustFit = true) {
  ...
  numInRect(rect, targets, mustFit = true) {
    let num = 0;
    for (var pos2 of this.iterRect(rect, mustFit)) {
      if (targets.includes(this.get(pos2))) num++;
    }
    return num;
  }
}
...
class InputHandler {
  ...
  process_touch(ev, name) {
    ...
    if (this.grabbed !== null) {
      for (let to of ev.changedTouches) {
        let pos2 = this.grabbed.appToChild([to.clientX, to.clientY]);
        let t = new Touch({ pos: pos2, state: name, nativeObject: to, nativeEvent: ev, identifier: to.identifier });
  ...
  process_mouse(ev, name) {
    ...
    if (this.mouseTouchEmulation) {
      ...
      if (this.grabbed !== null) {
        ...
        let pos2 = this.grabbed.appToChild(pos0);
        let t = new Touch({ pos: pos2, state: mapping[name], nativeObject: ev, identifier: -1 });
  ...
  process_wheel(ev, name) {
    ...
    this.app.requestFrameUpdate();
    if (this.grabbed != null) {
      let pos2 = this.grabbed.appToChild([this.mouseev.clientX, this.mouseev.clientY]);
      let t = new Touch({ pos: pos2, state: name, nativeObject: ev });
...
class Widget extends Rect {
  ...
  *iterByPropertyValue(property, value) {
   ...
   */
  appToWidget(pos2, recurse = true) {
    if (this.parent) {
      let pt = this.parent.getTransformRecurse().inverse().transformPoint(new DOMPoint(pos2[0], pos2[1]));
   ...
   * @returns {Array<number>}
   */
  appToChild(pos2, recurse = true) {
    let pt = this.getTransformRecurse().inverse().transformPoint(new DOMPoint(pos2[0], pos2[1]));
    return new Vec2([pt.x, pt.y]);
  }
  /**
  ...
  toChild(pos2) {
    ...
    if (!tx) return pos2;
    let pt = tx.inverse().transformPoint(new DOMPoint(pos2[0], pos2[1]));
    return new Vec2([pt.x, pt.y]);
  }
  /**
   ...
   */
  addChild(child, pos2 = -1) {
    if (pos2 == -1) {
      this._children.push(child);
    } else {
      this._children = [...this._children.slice(0, pos2), child, ...this._children.slice(pos2)];
    }
...
class LayeredTileMap extends TileMap {
   ...
   * @param {VecLike} pos 
   */
  getFromLayer(layer, pos2) {
    return this._layerData[layer][pos2[0] + pos2[1] * this.tileDim[0]];
  }
  /**
   ...
   * @param {number} val
   */
  setInLayer(layer, pos2, val) {
    this._layerData[layer][pos2[0] + pos2[1] * this.tileDim[0]] = val;
...
class WebGLRenderer {
  ...
  constructor(canvas) {
    ...
    if (this.gl === null) throw Error(`WebGL context couldb not be retrieved from ${canvas}`);
    ...
    this.vertexData = new Float32Array(100 * 6 * (2 + 2 + 4));
    this.indexData = new Uint16Array(100 * 6);
   ...
   */
  registerTexture(name, image, tileDim = 1, interp = null) {
    const gl = this.gl;
    ...
    this.textures.set(name, { texture, width: image.width / tw, height: image.height / th });
  }
  start(tileScale) {
    const gl = this.gl;
   ...
   * @returns 
   */
  drawTexture(name, sx, sy, sw, sh, dx, dy, dw, dh, tint2) {
    if (!this.textures.has(name)) return;
    const { texture, width, height } = this.textures.get(name);
    ...
    const u1 = sx / width;
    const v1 = sy / height;
    const u2 = (sx + sw) / width;
    const v2 = (sy + sh) / height;
    const color = tint2 != null ? tint2 : [1, 1, 1, 1];
    ...
    this.vertexCount += 4;
    if (this.vertexCount >= 100 * 4) this.flush();
  }
  flush() {
...
class WebGLWidget2D extends Widget {
  constructor(props = {}) {
    var _a, _b;
    super();
    this.drawBatch = [];
    const bgCanvas = document.createElement("canvas");
    this.webglRenderer = new WebGLRenderer(bgCanvas);
    this.txTileDim = (_a = props["tileDim"]) != null ? _a : 1;
    this.txImage = new Image();
    this.txImage.src = (_b = props["src"]) != null ? _b : "";
    this.txImage.onload = () => {
   ...
   * @param {[number, number, number, number]} tint 
   */
  addGlyph(sx, sy, sw, sh, dx, dy, dw, dh, tint2) {
    this.drawBatch.push({ sx, sy, sw, sh, dx, dy, dw, dh, tint: tint2 });
  }
  /**@type {Widget['draw']} */
  draw(app, ctx) {
...
class GLDemo extends App {
  ...
  on_key_down(e, o, v) {
    ...
    webgl.drawBatch[webgl.drawBatch.length - 1].dx = this.playerPos[0];
    webgl.drawBatch[webgl.drawBatch.length - 1].dy = this.playerPos[1];
    GLDemo.get().requestFrameUpdate();
  }
}
const glDemo = new GLDemo();
const webgl = new WebGLWidget2D({
  id: "webgl",
  src: urlTileset,
  txTileDim: 8,
  hints: { center_x: 0.5, y: 0.1, w: "16", h: "10" }
});
...
for (let i = 0; i < 160; ++i) {
  if (Math.random() > 0.5) {
    webgl.addGlyph(4, 4, 1, 1, i % 16, Math.floor(i / 16), 1, 1, tint);
  } else {
    webgl.addGlyph(5, 4, 1, 1, i % 16, Math.floor(i / 16), 1, 1, tint);
  }
}
const pos = [8, 5];
webgl.addGlyph(5, 0, 1, 1, pos[0], pos[1], 1, 1, tint);
glDemo.start();

    ...
    constructor(properties=null) {
        ...
        return new Proxy(this, {
            set(target, name, value) {
                if(name[0]=='_') return Reflect.set(...arguments);
...
export class ScrollView extends Widget {
    ...
    *iter(recursive=true, inView=true) {
    ...
    on_scrollX(event, value) {
        ...
        switch(this.wAlign) {
            case 'center':
                align = 0.5; // (this.children[0].w-this.w/this.zoom)/2;
                break
            case 'right':
                align = 1; // (this.children[0].w-this.w/this.zoom);
        }
        this._scrollX = this.scrollableW<=0 ? 
                        align : clamp(value,0,1); //this.children[0].w-this.w/this.zoom
    }
    on_scrollY(event, value) {
        ...
        switch(this.hAlign) {
            case 'middle':
                align = 0.5; //(this.children[0].h-this.h/this.zoom)/2;
                break
            case 'bottom':
                align = 1; //(this.children[0].h-this.h/this.zoom);
        }
        this._scrollY = this.scrollableH<=0 ? 
                        align : clamp(value,0,1); //this.children[0].h-this.h/this.zoom
    }
    to_local(pos) {
        return [(pos[0]-this.x)/this.zoom+this._scrollX*this.scrollableW, (pos[1]-this.y)/this.zoom+this._scrollY*this.scrollableH];
    }
    on_touch_down(event, touch) {
        this.vel = null;
        let r = touch.rect;
        ...
        if(this.collide(r)) {
            ...
            let millis = Date.now();
            this.oldTouch = [tl.x, tl.y, tl.identifier, millis, null];
    ...
    on_touch_up(event, touch) {
        if(this.oldTouch!=null) {
            this.vel = this.oldTouch[4];
        }
        this.oldTouch = null;
        this._lastDist = null;
        let r = touch.rect;
    ...
    update(millis) {
        super.update(millis);
        if(this.vel !== null) {
            this.scrollX += this.vel.x*millis/this.scrollableW;
            this.scrollY += this.vel.y*millis/this.scrollableH;
            this.vel = this.vel.scale(this.momentumDecay**(millis/30));
            let a=this.vel.abs();
            if(a.x<=this.momentumCutoff*this.scrollableW/this.zoom && a.y<=this.momentumCutoff*this.scrollableW/this.zoom) this.vel=null;
        }
    }
    on_touch_move(event, touch) {
        let r = touch.rect;
        ...
        if(this.collide(r)) {
            ...
            if(touch.nativeEvent==null || touch.nativeEvent.touches.length==1) { // || touch.nativeEvent.touches.length==2
                ...
                if(this.oldTouch!=null && touch.identifier==this.oldTouch[2]) {
                    if(this.scrollW) {
                        this.scrollX = this.scrollableW==0? 0 : (this._scrollX*this.scrollableW + (this.oldTouch[0] - tl.x))/this.scrollableW;
                    }
                    if(this.scrollH) {
                        this.scrollY = this.scrollableH==0? 0 : (this._scrollY*this.scrollableH + (this.oldTouch[1] - tl.y))/this.scrollableH;
                    }
                    //Need to recalc positions after moving scroll bars
                    let tln = touch.local(this);
                    let ot = this.oldTouch;
                    let time = Date.now()
                    let vel = new Vec2([0,0]), ovel = new Vec2([0,0]);
                    if(ot!=null) {
                        let millis = Math.max(time-ot[3], 1);
                        let vx = this.scrollableW>0? (tln.x-tl.x)/millis : 0;
                        let vy = this.scrollableH>0? (tln.y-tl.y)/millis : 0;
                        vel = new Vec2([vx, vy]);
                        ovel = ot[4]!==null ? ot[4] : ovel;
                    }
                    this.oldTouch = [tln.x, tln.y, touch.identifier, time, (vel.add(ovel)).scale(0.5)];
                }
            } 
            //Zoom
            if(this.uiZoom && touch.nativeEvent!=null && touch.nativeEvent.touches.length==2) {
                ...
                if(this._lastDist != null) {
                    ...
                    let sw = this._scrollX*this.scrollableW + this.w/this.zoom/2;
                    let sh = this._scrollY*this.scrollableH + this.h/this.zoom/2;
                    ...
                    this.scrollX = (sw + loc[0] - moc[0] - this.w/this.zoom/2)/this.scrollableW;
                    this.scrollY = (sh + loc[1] - moc[1] - this.h/this.zoom/2)/this.scrollableH;
                }
                this._lastDist = d;
            }
    ...
    on_wheel(event, touch) {
        let app = App.get();
        let sx = this._scrollX*this.scrollableW + this.w/this.zoom/2;
        let sy = this._scrollY*this.scrollableH + this.h/this.zoom/2;
        ...
        if(this.uiZoom && app.inputHandler.isKeyDown("Control")) {
            ...
            let lx = loc.x; //*this.scrollableW;
            let ly = loc.y; //*this.scrollableH;
            ...
            let mx = moc.x; //*this.scrollableW;
            let my = moc.y; //*this.scrollableH;
    
            this.scrollX = this.scrollableW==0? 0 : (sx+lx-mx - this.w/this.zoom/2)/this.scrollableW;
            this.scrollY = this.scrollableH==0? 0 : (sy+ly-my - this.h/this.zoom/2)/this.scrollableH;    
            ...
            this.scrollX += this.scrollableW==0? 0 : this.w * this.zoom * (wheel.deltaY/app.w) / this.scrollableW;
            if(this.scrollX!=this._scrollX) this.scrollX = this._scrollX;
        } else if(this.scrollH) {
            this.scrollY += this.scrollableH==0? 0 : this.h * this.zoom * (wheel.deltaY/app.h) / this.scrollableH;
            if(this.scrollY!=this._scrollY) this.scrollY = this._scrollY;
        }
    ...
    get scrollableW() {
        ...
        return this.children[0].w - this.w/this.zoom;
    }
    get scrollableH() {
        if(this.children.length==0) return 0;
        return this.children[0].h - this.h/this.zoom;
    }
    _draw() {
        this.draw();
        let r = this.rect;
        ...
        app.ctx.rect(r[0],r[1],r[2],r[3]);
        app.ctx.clip();
        app.ctx.translate(this.x-this._scrollX*this.scrollableW*this.zoom,
                        this.y-this._scrollY*this.scrollableH*this.zoom)

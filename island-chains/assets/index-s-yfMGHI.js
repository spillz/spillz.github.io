...
class ProductionChain extends Map {
  ...
  hasConnection(resource, connection) {
    var _a, _b;
    return (_b = (_a = this.get(resource)) == null ? void 0 : _a.includes(connection)) != null ? _b : false;
  }
  /**
   * 
   * @param {ResourceType} resource 
  ...
  removeConnection(resource, connection) {
    ...
    if (this.has(resource)) {
      const arr = (_a = this.get(resource)) != null ? _a : [];
      if (arr.length > 1) {
    ...
    while (changes && loops < 1e3) {
      ...
      for (let tile of placedTiles) {
        ...
        for (let terr of this.board.connectedIter(terr0, player, /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set())) {
          ...
          if (conn.has(adjTile) && !tile.needsFilled.meets(tile.needs))
            continue;
    ...
    for (let terr of this.board.connectedIter(terrain, player, /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set())) {
      ...
      if (terr.tile !== null) {
        if (terr !== terrain) {
          ...
          for (let n of tile.needsFilled.keys()) {
            if (((_c = tile.needsFilled.get(n)) != null ? _c : []).length < ((_d = tile.needs.get(n)) != null ? _d : 0)) {
              input = n;
              break;
            }
          }
          for (let n of tile.productionFilled.keys()) {
            if (((_e = tile.productionFilled.get(n)) != null ? _e : []).length < ((_f = tile.productionCapacity.get(n)) != null ? _f : 0)) {
              output = n;
              break;
            }
          }
